...
type serverHandshakeStateTLS13 struct {
	...
	// certPSK is the Cert PSK.
	certPSK         []byte
	handshakeTimings CFEventTLS13ServerHandshakeTimingInfo

	// WrappedCertsDB is the in-memory "database" of the server's wrapped certificates.
	// Certificates in the WrappedCertsDB are uniquely identified by the Cert PSK's identity/label.
...
// PKIELP modification: when in a PKIELP-enabled TLS handshake, the server will pick 
// the wrapped certificate correspondent to the Cert PSK extension's `identities` field.
	...
	if hs.clientHello.delegatedCredentialSupported && len(hs.clientHello.supportedSignatureAlgorithmsDC) > 0 {
		...
		if hs.keyKEMShare {
			if hs.sigAlg.isKEMTLS() {
				...
				c.didPQTLS = true
			} else if hs.c.config.WrappedCertEnabled && hs.c.config.PQTLSEnabled {
...
// sendServerFinished
// PKIELP modifications: after sending the server's Finished message, if the client
// is willing to establish a new Cert PSK (i.e. the Cert PSK extension's `establishPSK` field is true),
// the server will send the NewCertPSK message.
func (hs *serverHandshakeStateTLS13) sendServerFinished() error {
	...
	if !hs.requestClientCert() {
		...
		if c.config.WrappedCertEnabled && hs.clientHello.certPSK.establishPSK {
			fmt.Printf("The Client is willing to establish a new PSK. Sending a NewCertPSK message\n\n")  // HS Prints
...
// sendCertPSK construct and send the NewCertPSK handshake message.
// It also derives the Cert PSK from the Cert PSK Master Secret, storing it
// in the server's Cert PSK database.
func (hs *serverHandshakeStateTLS13) sendCertPSK() error {
	...
	fmt.Printf("NewCertPSK:\n  Nonce: %x\n  Label: %x\n\n", m.nonce[:10], m.label[:10])
...
// loadAllWrappedCerts loads to memory all wrapped certificates stored in the filesystem,
// storing them in `WrappedCertsDB`.
