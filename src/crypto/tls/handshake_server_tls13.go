...
func (hs *serverHandshakeStateTLS13) pickCertificate() error {
	...
	if c.config.WrappedCertEnabled && (len(hs.clientHello.certPSK.identities) > 0) {
		...
		fmt.Printf("Client sent a Cert PSK Label in it's ClientHello. Recovering the corresponding Wrapped Certificate\n\n")  // HS Prints
		...
		*certificate = hs.WrappedCertsDB[encodedIdentity]
		...
		fmt.Printf("Wrapped Certificate Proposal is enabled\n\n")  // HS Prints
		fmt.Printf("Picking a certificate for the handshake\n\n")  // HS Prints
		fmt.Printf("Client did not sent a Cert PSK Label in it's ClientHello. Using a classic certificate\n\n")  // HS Prints

		certificate = new(Certificate)
		*certificate = hs.WrappedCertsDB["normal_cert"]
		if certificate.Certificate == nil {
			return errors.New("pre quantum server certificate not found")
		}
	} else {
		certificate, err = c.config.getCertificate(clientHelloInfo(c, hs.clientHello))
		if err != nil {
...
func (hs *serverHandshakeStateTLS13) sendServerFinished() error {
	...
	if !hs.requestClientCert() {
		...
		if c.config.WrappedCertEnabled && hs.clientHello.certPSK.establishPSK {
			fmt.Printf("The Client is willing to establish a new PSK. Sending a newCertPSKMsgTLS13\n\n")  // HS Prints
...
func (hs *serverHandshakeStateTLS13) sendCertPSK() error {
	c := hs.c
	fmt.Printf("Sending the NewCertPSK message for the Client\n\n")  // HS Prints
	...
	fmt.Printf("Computed Cert PSK info:\n   Label (generated by the server): %x\n   Nonce (generated by the server): %x\n   Cert PSK: %x\n\n", m.label[:10], m.nonce[:10], psk[:10])  // HS Prints
